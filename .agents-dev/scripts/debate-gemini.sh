#!/usr/bin/env bash
# debate-gemini.sh — invoke Gemini as debate participant P2.
#
# Usage:
#   debate-gemini.sh r1 "question" outfile
#   debate-gemini.sh r2 "question" outfile other1-file other2-file
#   debate-gemini.sh r3 "question" outfile other1-file other2-file
#
...
# latest-gemini.log symlink so dashboard.sh can display live status.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AGENTS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
ROLE_FILE="$AGENTS_DIR/roles/debater-gemini.md"
...
mkdir -p "$LOG_DIR"
TS="$(date +%Y%m%d-%H%M%S)"
STRUCTURED_LOG="$LOG_DIR/gemini-debate-${ROUND}-${TS}.log"
ln -sfn "$STRUCTURED_LOG" "$LOG_DIR/latest-gemini.log"
...
---
다음 질문에 토론 참가자(P2: Gemini)로서 답하라.
...
# ── Write structured log header (dashboard-compatible) ───────────────────
{
  ...
  echo "=== QUERY ==="
  echo "[debate-gemini ${ROUND}] $QUESTION"
  echo "=== RESPONSE ==="
} > "$STRUCTURED_LOG"
